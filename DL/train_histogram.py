...
from PEDataSet_histogram import PEDataSet
import os
import torch
...
from Transformermodel_histogram import TransformerModel
...
def train(epoch,start_batch):
    ...
    for batch_idx, data in enumerate(progress_bar,start=start_batch+1):
        if batch_idx  <= len(progress_bar):
            ...
            if batch_idx % 1000 == 1:
                with open('training_log_histogram.txt', 'a') as file:
                    # Write the epoch number and loss to the file
                    if is_transformer:
                        file.write("transformer6-epoch=%d,batch_idx=%d, loss=%f,acc=%s\n" % (epoch,batch_idx, loss.item(),str(round(accuracy,4))))
                    else:
...
def save_model(model,epoch,total_loss,optimizer,batch_num):
    if is_transformer:
        name = '/home/lizijian/lincode/ember/model_data/transformer6-model-' + str(epoch) + '-' + str(batch_num) + '-' +str(round(total_loss.item(),4) )
    else:

        name = '/home/lizijian/lincode/ember/model_data/cnnmodel-' + str(epoch) + '-' + str(round(total_loss.item(),4) )
...
if __name__ == '__main__':
    ...
    test_dataset_path = conf.test_dataset_path
    ...
    is_gen_traindataset = True
    is_gen_testdataset = True
    is_load_model = False
    is_train = True
    path_to_model = conf.train_load_model_path
    start_epoch = -1
